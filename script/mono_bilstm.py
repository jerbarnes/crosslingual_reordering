...
def get_best_weights(lang, classifier='bilstm', binary=False, mono=False):
    if binary:
        if mono:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--lang', default='en', help='choose language: en, es, ca (defaults to en)')
    ...
    # get vocabulary and maximum sentence length
    ...
    if args.binary:
        paramfile = 'models/artetxe-bilstm/binary-{0}/{0}-monolingual-w2idx.pkl'.format(args.lang)
    else:
        paramfile = 'models/artetxe-bilstm/4class-{0}/{0}-monolingual-w2idx.pkl'.format(args.lang)
