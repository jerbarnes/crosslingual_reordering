...
\alex{this 'we...' means a small description of the experiments? Should this be a summary of 3.2 'Experimental Setup'?}
...
\cite{Mohammad2015b}
\alex{what is the context of this reference?}
...
Because of some problems we may find in MT, sometimes it is considered best to preprocess the source language by reordering it and then carrying out the translation (\newcite{Bisazza2016}). Transformation (reordering) rules can be determined manually or with data-driven approaches. Their application can be deterministic or non-deterministic. Some hybrid techniques exist as well, where long-range transformations are deterministic and the rest non-deterministic.
...
\alex{regarding the table: Shouldn't we change the examples on lexicon? I mean that the length of Only-Lexicon is only 3, but what we are actually experimenting with is 'UNK UNK negativo...', right?}
...
\alex{what exactly is missing here?}
...
In order to test our hypothesis, we compare three different classifiers: a Support Vector Machine (SVM) with Bag-of-Embeddings, a Convolutional Neural Network (CNN), and a Bidirectional Long Short Term Memory Network (BiLSTM). Each of these classifiers theoretically have an increasing reliance on word order. The SVM does not take into account word order at all and is therefore our order-agnostic baseline. An SVM can be considered a strong baseline for SA (\newcite{Kiritchenko2014c}). The CNN considers only local word order, and in this respect is basically a parameterized n-gram model (\newcite{Barnes2017}, \newcite{Santos2014}, \newcite{Severyn2015}). Finally, the BiLSTM considers both short-term and long-term word order in both directions. An LSTM is a type of Recurrent Neural Network that takes in particular consideration sequential information. LSTMs currently produce state-of-the-art results in SA (\cite{Barnes2017}, \newcite{Howard2018}, \newcite{Tai2015a}).

For all models, we optimize the parameters on the source-language development set and test on the target-language data.

\jer{Could you cite the relevant papers for each of these and give more details on their assumptions regarding word order? It would also be useful to explain the training process and hyperparameter optimization.}
\alexanswer{relevant papers done; regarding more on word order, optimization and training, I don't have really more on the thesis, what else should I put here?}

\section{Results}

\todo{color appropriate boxes}
...
\multirow{4}{*}{\rt{Mono}}
	& \multirow{4}{*}{\rt{EN}}
  		& Original & 0.826 & 0.763 & 0.708 & 0.502 & 0.489 & 0.449 \\ 
		&& Random & 0.782 & 0.750 & 0.708 & 0.457 & 0.445 & 0.449 \\ 
		&& Only Lexicon  & 0.632 & 0.607 & 0.491 & 0.370 & 0.331 & 0.389 \\ 
		&& No Lexicon & 0.735 & 0.705 & 0.658 & 0.465 & 0.450 & 0.435 \\ 
...
\multirow{8}{*}{\rt{Machine Translation}}
	& \multirow{4}{*}{\rt{EN (ES)}}
  		& Original & 0.713 & 0.671 & 0.696 & 0.481 & 0.429 & 0.451 \\ 
		&& Random & 0.710 & 0.659 & 0.696 & 0.480 & 0.417 & 0.451 \\ 
		&& Only Lexicon  & 0.618 & 0.473 & 0.545 & 0.322 & 0.273 & 0.357 \\ 
		&& No Lexicon & 0.632 & 0.591 & 0.654 & 0.429 & 0.379 & 0.420 \\ 
	\sepp
	& \multirow{4}{*}{\rt{EN (CA)}}
  		& Original & 0.752 & 0.742 & 0.709 & 0.487 & 0.421 & 0.447 \\ 
		&& Random & 0.761 & 0.757 & 0.747 & 0.463 & 0.416 & 0.455 \\ 
		&& Only Lexicon  & 0.521 & 0.596 & 0.439 & 0.261 & 0.301 & 0.327 \\ 
		&& No Lexicon & 0.730 & 0.715 & 0.708 & 0.467 & 0.431 & 0.444 \\ 
...
\todo{Things from the table of results that should (I think) be discussed: even with the "new" way to do the lexicon experimenting, bilstm behaves very differently than the other two, mainly in Only Lexicon}
\todo{Difference between only lexicon and no lexicon in bilstm still does not seem as significant as it should be (right?)}
\todo{why does catalan behave almost invariable better than spanish, even in MT versions?}
\todo{why does the cnn not take into account word order at all but reacts so badly in Only Lexicon? What is making this drop?}
...
\alex{should we delete this section?}
